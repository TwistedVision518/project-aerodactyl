...
function App() {
  ...
  const [activeMobileRom, setActiveMobileRom] = useState<RomEntry | null>(null)
  ...
  const comparedRoms = useMemo(
    () => roms.filter((rom) => comparisonIds.includes(toSectionId(rom.name))),
    [comparisonIds],
  )
  ...
  const scrollSelectedRomIntoView = () => {
    const scrollBehavior: ScrollBehavior =
      prefersReducedMotion || isMobileDevice() ? 'auto' : 'smooth'
  ...
  const handleRomAnchorClick = (event: ReactMouseEvent<HTMLAnchorElement>, romId: string) => {
    ...
    const rom = roms.find(r => toSectionId(r.name) === romId)
    if (isMobileDevice() && rom) {
      setActiveMobileRom(rom)
      return
    }
  ...
  const handleToggleComparison = (romId: string, event?: ReactMouseEvent) => {
    if (event) event.stopPropagation()
    setComparisonIds(prev => 
      prev.includes(romId) 
        ? prev.filter(id => id !== romId) 
        : prev.length < 3 ? [...prev, romId] : prev
    )
  }
    ...
    <>
      ...
      <LazyMotion features={domAnimation}>
        <div className="app-shell scene-root">
                ...
                <div className="command-sidebar">
                  <article className="side-card surface-utility">
                    ...
                    <PulseTimeline 
                      ...
                      onUpdateClick={handleLatestUpdateClick} 
                    />
                  </article>
                </div>
              </div>
          ...
          <Reveal delay={90}>
            <section className="explorer panel" data-hub-accent="true" id="rom-directory" style={featuredStyle}>
              <div className="section-heading">
                ...
                <p>Search the lineup, compare builds side-by-side, then open one focused release sheet.</p>
              </div>
              ...
              <m.div className="rom-directory-grid" layout>
                ...
                {filteredRoms.map((rom, index) => {
                  ...
                  const isCompared = comparisonIds.includes(romId)
                  const accentStyle: AccentStyle = {
                    '--accent': rom.accent,
                    '--accent-soft': rom.accentSoft,
                    '--accent-strong': rom.accentStrong,
                  ...
                  return (
                    ...
                    >
                      <ReactivePanel
                        ...
                        className={`rom-directory-item surface-signal ${isActiveRom ? 'is-active' : ''} ${isCompared ? 'is-compared' : ''}`.trim()}
                      ...
                      >
                        ...
                        <div className="directory-topline">
                          ...
                          <button 
                            className={`compare-checkbox ${isCompared ? 'is-checked' : ''}`}
                            onClick={(e) => handleToggleComparison(romId, e)}
                            aria-label="Compare this ROM"
                          >
                            {isCompared ? '✓' : '+'}
                          </button>
                        </div>
          ...
          <BottomSheet 
            isOpen={activeMobileRom !== null} 
            onClose={() => setActiveMobileRom(null)}
            title={activeMobileRom?.name}
          >
            {activeMobileRom && (
              <div className="mobile-rom-details">
                <div className="chip-row">
                  <span className="chip chip-tonal">{activeMobileRom.status}</span>
                  <span className="chip">{activeMobileRom.branch}</span>
                </div>
                <p className="mobile-rom-tagline">{activeMobileRom.tagline}</p>
                
                <div className="mobile-rom-specs">
                  <div className="rom-fact-row">
                    <span>Version</span>
                    <strong>{activeMobileRom.version}</strong>
                  </div>
                  <div className="rom-fact-row">
                    <span>Build date</span>
                    <strong>{activeMobileRom.buildDate}</strong>
                  </div>
                </div>

                <div className="mobile-rom-actions">
                  {getReleaseLinks(activeMobileRom).map(link => (
                    <a key={link.url} href={link.url} className="action-primary" target="_blank" rel="noreferrer">
                      {link.label}
                    </a>
                  ))}
                </div>

                <div className="mobile-rom-notes">
                  <span className="section-label">Highlights</span>
                  <ul className="bullet-list">
                    {activeMobileRom.highlights.map(h => <li key={h}>{h}</li>)}
                  </ul>
                </div>
              </div>
            )}
